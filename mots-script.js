(function () {
  ...
  const elNature = document.getElementById("nature-tag");
  ...
  function render() {
    const entry = MOTS[currentIndex];

    elExprText.textContent = entry.mot;
    elNature.textContent = entry.nature;
    elExplication.textContent = entry.definition;
  ...
  function buildArchive(filter = "") {
    ...
    MOTS.forEach((entry, i) => {
      if (needle && !entry.mot.toLowerCase().includes(needle)) return;
      const li = document.createElement("li");
      const btn = document.createElement("button");
      btn.textContent = entry.mot;
